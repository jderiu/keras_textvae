...
def main(args):
    ...
    with open(config_fname, 'r') as json_data:
        ...
        # == == == == == == == == == == =
        train_model, test_model, discriminator = vae_model(config_data, vocab, step)
        ...
        discriminator.compile(optimizer=optimizer, loss='categorical_crossentropy') #output of the discriminator model are the outputs -> specifiy cross_entropy as loss
        ...
        model_checkpoint = ModelCheckpoint('models/vae_model/discr_weights.hdf5', save_weights_only=True, save_best_only=True, monitor='val_loss', mode='min', verbose=1)
        logging.info('Pretrain the Discriminators')
        history = discriminator.fit(
            x=disX_train,
            y=disy_train,
            validation_data=(valid_input[-1], valid_input[:9]),
            epochs=1000,
            batch_size=1024,
        ...
        for i, (loss, val_loss) in enumerate(zip(losses, val_losses)):
            logging.info('Epoch: {} Loss: {} Val Loss: {}'.format(i, loss, val_loss))
        ...
        lex_output2 = LexOutputCallbackGAN(test_model, valid_dev_input2, valid_dev_lex2, 1, vocab, delimiter, fname='{}/test_output_located'.format(log_path))
        lex_output3 = LexOutputCallbackGAN(test_model, valid_dev_input3, valid_dev_lex3, 1, vocab, delimiter, fname='{}/test_output_random'.format(log_path))
